    ...
    return (
      <div className="flex flex-col gap-2 p-2 md:p-4 rounded-lg">
          <div className="text-gray-800 text-lg md:text-2xl bg-white text-blue-900 border-2 border-blue-900 p-2 rounded-lg text-center w-full font-bold">
            ...
            <p onClick={() => setShowRanking(!showRanking)} className="underline text-xs md:text-sm cursor-pointer">{showRanking ? "Ocultar ranking" : "Ver ranking completo"}</p>
            {showRanking && ranking.length > 0 && (
                <div className="flex flex-col gap-1 md:gap-2 items-start text-left px-1 mt-2">
                    {ranking.map((item, index) => (
                        <p key={index} className={`${index >= 3 ? 'text-xs md:text-sm' : 'text-sm md:text-lg'} truncate w-full`}>
                            {index + 1}. {item}
                        </p>
                    ))}
                </div>
